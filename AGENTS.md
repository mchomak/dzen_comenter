...
- `OrchestratorLoop._generate_batch` генерирует только элементы, присутствующие в текущем DOM-снимке; для недоступных сохраняет `ERROR` и не вызывает `DzenPage.publish_reply`. Он восстанавливает исходные номера и сохраняет полный набор outcomes в исходном порядке. `save_batch_outcomes` атомарно валидирует этот порядок, записывает reply/status и при `ERROR` повторно ставит элемент в очередь до `batch_max_attempts_per_comment` с задержкой `batch_retry_cooldown_minutes`; batch получает `error`, если ошибся хотя бы один элемент.
