...
| Команда | Назначение |
|---|---|
| `.venv\Scripts\python.exe -m pytest -q` | Полный тестовый набор: проверено 2026-09-05, `450 passed, 34 skipped, 1 warning`. |
...
- [dzen_commenter/orchestrator/loop.py](dzen_commenter/orchestrator/loop.py) восстанавливает либо получает авторизацию, читает и дедуплицирует комментарии, соблюдает ограничения по возрасту и числу публикаций в час, строит prompt, сохраняет reply и публикует его только при `auto_publish`. Перед каждым AI-вызовом цикл сравнивает live `max_comments_per_hour` со счётчиком результатов AI за последний час Москвы; при достижении лимита прекращает обработку.
...
- Пакетные ответы хранятся в `reply_batches`, `comment_batch_queue` и `reply_batch_items` (миграция [0008_add_reply_batches.py](dzen_commenter/db/migrations/versions/0008_add_reply_batches.py)). [dzen_commenter/contracts/models.py](dzen_commenter/contracts/models.py) определяет `BatchItem`, `ClaimedBatch` и `BatchOutcome`; шов репозитория — в [dzen_commenter/contracts/interfaces.py](dzen_commenter/contracts/interfaces.py).
- При batch-режиме `OrchestratorLoop._run_batch_cycle` ставит свежие eligible-комментарии с `post_url` в очередь, затем передаёт лимиты и часовую квоту в `claim_next_batch`; элементы без `post_url` обрабатывает старым одиночным путём. Режим активен только если `batch_replies_enabled` и `batch_cutover_at` — непустой ISO-8601 datetime с timezone; иначе парсер runtime-конфига принудительно выключает режим.
...
- `OrchestratorLoop._generate_batch` генерирует только элементы, присутствующие в текущем DOM-снимке; недоступные в снимке сохраняет как `SKIP` без повторной постановки в очередь и не вызывает `DzenPage.publish_reply`. Он восстанавливает исходные номера и сохраняет полный набор outcomes в исходном порядке. `save_batch_outcomes` атомарно валидирует этот порядок, записывает reply/status и при настоящем `ERROR` повторно ставит элемент в очередь до `batch_max_attempts_per_comment` с задержкой `batch_retry_cooldown_minutes`; batch получает `error`, если ошибся хотя бы один элемент.
...
Для защитного лимита используйте `CommentRepository.count_ai_attempts_since(since)`: SQL считает ответы со статусами `generated`, `published` и `error`, у которых `created_at >= since`, поэтому черновики и ошибки тоже расходуют лимит. Проверяйте его через `OrchestratorLoop.run_cycle` с fake repository/provider из [tests/orchestrator/conftest.py](tests/orchestrator/conftest.py); границу ветки — через `DameoPromptBuilder.build(PromptContext(...))` в [tests/prompt/test_builder.py](tests/prompt/test_builder.py). Пакетную очередь проверяйте через [tests/orchestrator/test_loop.py](tests/orchestrator/test_loop.py), [tests/db/test_repository.py](tests/db/test_repository.py) и [tests/prompt/test_batch.py](tests/prompt/test_batch.py): `.venv\Scripts\python.exe -m pytest tests/orchestrator/test_loop.py tests/db/test_repository.py tests/prompt/test_batch.py -q` — проверено 2026-09-05, `102 passed, 34 skipped`; полный набор — `.venv\Scripts\python.exe -m pytest -q` (`450 passed, 34 skipped, 1 warning`).
